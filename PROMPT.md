...
## Rules of Conduct (apply to ALL phases)
...
- Each phase writes its findings to a file (`phase1_results.md`, `phase2_results.md`, etc.) as soon as results are available. Do not wait until the end.

---

## Phase 1: Preparation

### 1.1 Find the main tex file

Find the `.tex` file containing `\begin{document}` (it may not be named `main.tex`). Read the full `.tex` source and all `.bib` files.

### 1.2 Ensure a compiled PDF exists
...
## Phase 2: Download Guides + Plan + Write Python Script

### 2.1 Download and read the reference guides
...
### 2.3 Write the Python checking script
...
- **Write results to `phase2_results.md` immediately.**

**Note: The document is constantly being edited. Re-read files every time you check. Do not rely on cached content.**

---

## Phase 3: LLM Full-Text Review

For all items in the LLM group, plus all items Phase 2 already checked (double-check), perform an LLM full-text review.

**IMPORTANT: Each invocation must check at most 5 rules.** Split all rules into batches of 5. Launch a separate invocation for each batch (e.g., using subagents), and **run them in parallel**. Do NOT try to check all rules in a single pass.
...
## Phase 4: Section-by-Section Holistic Review
...
**Write results to `phase5_results.md` as each invocation completes.**

---

## Phase 6: Generate Report

**This phase only merges and formats.** All findings should already be recorded in `phase2_results.md` through `phase5_results.md`. Read those files, merge them into the final report structure, and format for output. Do not re-do any checks.
...
2. **Phase 2 Results: Python Automated Checks** — grouped by category
3. **Phase 3 Results: LLM Review** — content, language, visual, layout
4. **Phase 4 Results: Section-by-Section Holistic Review** — each section's weaknesses (ranked) and strengths
5. **Phase 5 Results: Reference Verification** — per-entry results with verification URLs
